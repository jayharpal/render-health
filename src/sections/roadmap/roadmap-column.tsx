...
import { getTaskBoards, updateTask } from 'src/redux/slices/taskboard';
//
import { useAuthContext } from 'src/auth/hooks';
import KanbanTaskItem from './roadmap-task-item';
...
export default function KanbanColumn({ column, tasks, index, projectId }: Props) {
  ...
  //       if (res?.data?.status) {
  //         dispatch(getTaskBoards(projectId as string));
  //       }
  //       openAddTask.onFalse();
  //     } catch (error) {
  //       console.error(error);
  //     }
  //   },
  //   [column.id, openAddTask, projectId, dispatch]
  // );
  ...
  return (
    <Draggable draggableId={column.id} index={index}>
      {(provided, snapshot) => (
        ...
        >
          <Stack {...(isAdmin ? provided.dragHandleProps : {})}>
