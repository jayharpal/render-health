...
type Props = {
  contacts: IUsers[];
  onAddRecipients: (selected: IUsers) => void;
};
  ...
  return (
    <>
      ...
      <Autocomplete
        ...
        onChange={(event, newValue) => handleAddRecipients(newValue as IUsers)}
        onInputChange={(event, newValue) => setSearchRecipients(newValue)}
        options={contacts}
        getOptionLabel={(recipient) => recipient?.name || recipient?.email}
          ...
          <li {...props} key={recipient._id}>
            ...
            >
              <Avatar
                ...
                src={recipient?.name || recipient?.email}
                sx={{ width: 1, height: 1 }}
              />
              <Stack
                alignItems="center"
        ...
        renderTags={(selected, getTagProps) =>
          selected.map((recipient, index) => (
            <Chip
              ...
              avatar={<Avatar alt={recipient?.name || recipient?.email} src={recipient?.name || recipient?.email} />}
              size="small"
              variant="soft"
            />
          ))
