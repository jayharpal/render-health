...
export default function InternNewEditForm({ currentIntern }: Props) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      role: currentIntern?.role || '',
      email: currentIntern?.email || '',
      state: currentIntern?.state || '',
      status: currentIntern?.status || '',
      address: currentIntern?.address || '',
      country: currentIntern?.country || '',
      zipCode: currentIntern?.zipCode || '',
      company: currentIntern?.company || '',
      avatarUrl: currentIntern?.avatarUrl || null,
      phoneNumber: currentIntern?.phoneNumber || '',
      isVerified: currentIntern?.isVerified || true,
    }),
    [currentIntern]
  );
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      router.push(paths.dashboard.intern.list);
