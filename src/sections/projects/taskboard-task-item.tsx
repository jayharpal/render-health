...
import KanbanDetails from './taskboard-details';
...
export default function KanbanTaskItem({
  ...
  const renderPriority = (
    <Iconify
      icon={
        (task.priority === 'lowest' && 'solar:double-alt-arrow-down-bold-duotone') ||
        ...
        (task.priority === 'highest' && 'solar:double-alt-arrow-up-bold-duotone') ||
  ...
  const renderInfo = (
    <Stack direction="row" alignItems="center" justifyContent="flex-end">
      ...
      >
        {task.assignee?.map((user) => <Avatar key={user.id} alt={user.name} src={user.name} />)}
      </AvatarGroup>
    </Stack>
  );
